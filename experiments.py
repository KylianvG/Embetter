...
def print_details():
    ...
    print("#" + "#".rjust(55))
    print("# " + f"Do soft debiasing from scratch: {str(FLAGS.do_soft)}".ljust(53) + "#")
...
def soft(E, embed, gender_words, defs):
    ...
    if FLAGS.do_soft:
        ...
        E_soft = deepcopy(E)
        soft_debias(E_soft, gender_words, defs,
...
def run_benchmark(E, E_hard, E_soft, embed):
    ...
    result_hard = benchmark.evaluate(E_hard, "'Hard debiased', {}".format(embed), print=False)
    results = None
    if E_soft:
        result_soft = benchmark.evaluate(E_soft, "'Soft debiased', {}".format(embed), print=False)
        results = [result_original, result_hard, result_soft]
        benchmark.pprint_compare(results, ["Before", "Hard-debiased", "Soft-debiased"], embed)
